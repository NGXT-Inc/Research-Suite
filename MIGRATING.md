...
v0.0012, and the hosted brain now requires a RapidReview account and a
project-scoped key. Your data carries over untouched — the switch is a plugin
swap plus minting a key.
...
| Binaries | `research-plugin-*` | `merv-client`, `merv-http` (no local MCP proxy — clients connect to the brain over HTTP) |
...
- The `.research_plugin/` (or `.merv/`) folder inside a project checkout is
  no longer consulted for project scope — the project is bound by your key,
  not by a folder link. Leaving the folder in place is harmless, and sandbox
  SSH keypairs stored there remain usable.
...
- Projects, claims, experiments, reviews, and reflections live in the
  hosted brain, keyed by project id. Your client reconnects to a project
  through its key; nothing folder-local is involved.
- Running sandboxes and detached runs are unaffected — the swap is entirely
  client-side, and existing sandbox SSH keys keep working.
...
Restart Claude Code, then [connect with a project key](#connect-with-a-project-key).
If you already run `merv@rapidreview`, this is just `claude plugin marketplace
update rapidreview && claude plugin update merv@rapidreview` (the full
...
restart Cursor. Then [connect with a project key](#connect-with-a-project-key).
...
directory (the old `research-plugin-mcp` no longer exists):

```bash
git -C /path/to/your-clone pull   # old remote URLs redirect
```

Then in `~/.codex/config.toml`, replace the old stdio `research-plugin`
server entry with a direct HTTP entry pointed at the brain — there is no
local proxy command anymore:

```toml
[mcp_servers.merv]
url = "https://experiments.rapidreview.io/mcp"
bearer_token_env_var = "MERV_MCP_KEY"
```

The committed `.mcp.codex.json` in the plugin carries the same registration;
see [merv/docs/CLIENTS.md](merv/docs/CLIENTS.md) for the current per-client
snippets. Restart Codex, then
[connect with a project key](#connect-with-a-project-key).
...
Open your research checkout and ask the agent to call
`project(action="current")` — it should return the key's bound project and
its id. Then `workflow.status_and_next(project_id)` picks up wherever you
left off.

## If something fails

- **Authorization errors on the old plugin** are expected — the hosted
  brain now requires a key. Migrating fixes them.
- **Empty project list in the UI**: you signed in with an email that
  isn't a member of your projects. Sign in with the right account, or ask
  your project owner to add your email — then mint the key from that
  account. Still stuck? Open an issue on
