...
from merv.brain.kernel.secret_tokens import hash_secret, mint_secret, secret_digest_matches


class SecretTokensTest(unittest.TestCase):
    def test_mint_secret_applies_prefix_and_entropy_size(self) -> None:
        with patch("merv.brain.kernel.secret_tokens.secrets.token_urlsafe", return_value="abc") as token_urlsafe:
    ...
    def test_missing_stored_digest_still_burns_compare(self) -> None:
        ...
        with patch("merv.brain.kernel.secret_tokens.hmac.compare_digest", return_value=True) as compare:
            self.assertFalse(
