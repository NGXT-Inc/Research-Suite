"""Functional tests for the rp_run launch wrapper.

These run the real RP_RUN_SCRIPT with sh against a temp directory standing in
for $RP_EXPERIMENT_DIR. The contract under test:

- a run detaches (rp_run returns while the command still executes) and the
...
class RpRunHarness(unittest.TestCase):
    def setUp(self) -> None:
        self.tmp = tempfile.TemporaryDirectory()
        self.experiment_dir = Path(self.tmp.name) / "exp"
        self.experiment_dir.mkdir()
        self.script = Path(self.tmp.name) / "rp_run"
        self.script.write_text(RP_RUN_SCRIPT)
        self.script.chmod(self.script.stat().st_mode | stat.S_IXUSR)

    def tearDown(self) -> None:
        self.tmp.cleanup()

    def rp_run(self, *args: str, experiment_dir: str | None = None):
        env = dict(os.environ)
        if experiment_dir is None:
            env["RP_EXPERIMENT_DIR"] = str(self.experiment_dir)
        else:
            env.pop("RP_EXPERIMENT_DIR", None)
            if experiment_dir:
                env["RP_EXPERIMENT_DIR"] = experiment_dir
    ...
    def test_success_run_writes_receipts(self) -> None:
        result = self.rp_run("prep", "--", "sh", "-c", "echo hello out")
    ...
    def test_newline_in_argument_keeps_meta_json_valid(self) -> None:
        ...
        result = self.rp_run("nl", "--", "sh", "-c", "echo one\necho two")
        self.assertEqual(result.returncode, 0, result.stderr)
    ...
    def test_failure_exit_code_is_recorded_by_the_wrapper(self) -> None:
        result = self.rp_run("boom", "--", "sh", "-c", "echo pre; exit 3")
        self.assertEqual(result.returncode, 0, result.stderr)
    ...
    def test_run_is_detached_and_outlives_the_launcher(self) -> None:
        # rp_run must return promptly while the command is still executing —
        # the SSH channel (here: the subprocess) is gone before the sentinel.
        started = time.monotonic()
        result = self.rp_run("slow", "--", "sh", "-c", "sleep 1; echo done")
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertLess(time.monotonic() - started, 1.0, "rp_run did not detach")
    ...
    def test_duplicate_label_is_refused(self) -> None:
        first = self.rp_run("dup", "--", "sh", "-c", "true")
        self.assertEqual(first.returncode, 0, first.stderr)
        second = self.rp_run("dup", "--", "sh", "-c", "true")
        self.assertEqual(second.returncode, 2)
        self.assertIn("already exists", second.stderr)

    def test_bad_usage_and_bad_label_are_refused(self) -> None:
        self.assertEqual(self.rp_run("nolabel").returncode, 2)
        self.assertEqual(self.rp_run("x", "echo", "hi").returncode, 2)  # no --
        bad = self.rp_run("bad/label", "--", "true")
        self.assertEqual(bad.returncode, 2)
        self.assertIn("label", bad.stderr)

    def test_missing_experiment_dir_is_refused(self) -> None:
        result = self.rp_run("x", "--", "true", experiment_dir="")
        self.assertNotEqual(result.returncode, 0)

    def test_listing_command_round_trips_through_the_parser(self) -> None:
        self.rp_run("done1", "--", "sh", "-c", "exit 7")
        self.rp_run("live1", "--", "sh", "-c", "sleep 30")
