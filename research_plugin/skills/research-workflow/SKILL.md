...
experiment, review, job, or attempt.
...
9. Use MCP jobs for expensive, long-running, resumable, or policy-sensitive ML runs.
...
MCP jobs run on the configured execution backend. The default backend is Modal
with an NVIDIA H100 GPU. Design experiments and runtime expectations assuming an
H100 unless the active deployment or the job's `backend_hints.gpu` explicitly
states otherwise. Use `backend_hints.gpu` only when there is a concrete reason
to override the default.

Use the top-level `env` field for safe job environment variables such as
`PYTHONUNBUFFERED`. Do not put `env`, free-form `note` text, or provenance
comments inside `backend_hints`; reserve `backend_hints` for documented backend
configuration such as `gpu`, `compute_tier`, `timeout`, `cuda_devel`,
`image_packages`, `experiment_path`, `cloud`, and `region`.
...
- associate synced resources with the current experiment, claim, job, or review
- when `workflow.status_and_next` includes `resource_guidance`, follow its
  `association_role`, `expected_output_paths`, and `job_id`; do not guess plural
  role names such as `results`, `report`, or `output`
...
review to MCP. They must not mutate claims, experiments, resources, jobs, or
workflow state.
...
- conclusion is grounded in files or job outputs
