...
def _reaper_capable_backend_factory(_repo: Path) -> FakeSandboxBackend:
    """A fake backend that passes the reaper/auto-sync backend-name gate."""
    backend = FakeSandboxBackend()
    backend.capabilities = BackendCapabilities(name="lambda_labs")
    return backend
...
class RouterRestartReaperTest(unittest.TestCase):
    ...
    def test_restart_resumes_reaper_for_project_with_running_sandbox(self) -> None:
        ...
        self.assertIsNotNone(app.sandboxes._reaper_thread)
        self.assertTrue(app.sandboxes._reaper_thread.is_alive())
