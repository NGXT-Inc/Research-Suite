...
6. Sandboxes
   ...
   - render the **MLflow** and **TensorBoard** dashboards from the row's
     `dashboards` map (provider URLs or daemon-owned local SSH forwards). One
     `<iframe>` tab per non-empty entry, sitting next to "Terminal." The URLs can
     change if a tunnel relocates or is recreated — re-read the row on the
     existing poll cadence rather than caching them in component state.
